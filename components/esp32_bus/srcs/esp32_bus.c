...
esp_err_t esp32_spi_bus_init (spi_host_device_t host, uint8_t sclk , uint8_t miso, uint8_t mosi)
{
    ...
    if (spi_bus_initialize(host, &spi_bus_cfg, SPI_DMA_CH_AUTO) != ESP_OK) 
        return ESP_FAIL;

    return ESP_OK;
}

esp_err_t esp32_spi_device_init (uint8_t bus, uint8_t cs)
{
...
esp_err_t esp32_spi_bus_deinit(spi_host_device_t host) {

    if (spi_bus_free(host) != ESP_OK){
            return ESP_FAIL;
        }
    return ESP_OK;
}

esp_err_t esp32_spi_device_deinit (uint8_t cs) {
